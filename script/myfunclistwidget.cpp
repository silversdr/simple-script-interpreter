...
MyFuncListWidget::MyFuncListWidget()
{
    this->setContextMenuPolicy( Qt::CustomContextMenu);
        ...
        m_actionTwo->setText("卸载");
        p_actionOne->setText("新建函数");
        p_actionTwo->setText("载入函数");
...
//设置menu和action
        ...
        connect(m_actionTwo, SIGNAL(triggered()), this, SLOT(actionUnload()));
...
void MyFuncListWidget::showMenu(const QPoint& point)
{
...
void MyFuncListWidget::actionSave()
{
    qDebug() << "actionOne"<<this->currentItem()->text();//第二种方法，通过currentItem方法获取选中的对象
    //，也可以正常输出，说明这种方法也可以获取对象内存指针
    emit save(this->currentItem());
}

void MyFuncListWidget::actionUnload()
{
...
void MyFuncListWidget::actionNew()
{
    emit newFunc();
}
void MyFuncListWidget::actionLoad()
{
    emit loadFunc();
}
